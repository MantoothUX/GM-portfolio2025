...
async function uploadImage(filePath: string): Promise<ImageMapping> {
  ...
  if (fileStats.size > 10 * 1024 * 1024) {
    console.log(`  Skipping large file (${(fileStats.size / 1024 / 1024).toFixed(2)}MB): ${contentPath}`);
    return {
...
async function main() {
  ...
  // Save account hash to .env.local if found
  if (accountHash) {
  ...
  const skippedCount = mapping.filter((m) => !m.uploaded).length;

  console.log('\nUpload Summary:');
  console.log(`  Uploaded: ${uploadedCount}`);
  console.log(`  Skipped (large files): ${skippedCount}`);
