...
const Navigation = ({
  ...
  return <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'py-4 bg-white/90 backdrop-blur-md border-b border-black' : 'py-6 bg-transparent'}`}>
      <div className="container mx-auto px-6 flex justify-between items-center">
        <button onClick={() => setCurrentView('home')} className="text-2xl font-black tracking-tighter uppercase italic flex items-center gap-2">
        ...
        <div className="hidden md:flex items-center gap-8 font-bold text-sm tracking-wider uppercase">
          <button onClick={() => setCurrentView('home')} className={`hover:text-fuchsia-600 transition-colors ${currentView === 'home' ? 'text-fuchsia-600 underline decoration-2 underline-offset-4' : 'text-black'}`}>
            Work
          </button>
          <button onClick={() => setCurrentView('about')} className={`hover:text-cyan-600 transition-colors ${currentView === 'about' ? 'text-cyan-600 underline decoration-2 underline-offset-4' : 'text-black'}`}>
            About Us
          </button>
...
export const BrandDesignStudio = () => {
  ...
  return <div className="font-sans antialiased bg-gray-50 min-h-screen selection:bg-fuchsia-300 selection:text-fuchsia-900">
      <Navigation currentView={currentView} setCurrentView={setCurrentView} isScrolled={isScrolled} />
